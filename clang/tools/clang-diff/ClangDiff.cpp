...
getAST(const std::unique_ptr<CompilationDatabase> &CommonCompilations,
  ...
  std::array<std::string, 1> const Files = {{std::string(Filename)}};
...
static unsigned printHtmlForNode(raw_ostream &OS, const diff::ASTDiff &Diff,
  ...
  diff::NodeId const TargetId = Diff.getMapped(Tree, Id);
  if (IsLeft) {
    MyTag = 'L';
    OtherTag = 'R';
    LeftId = Id;
  ...
  OS << "\n" << LeftId << " -> " << RightId;
  std::string const Value = Tree.getNodeValue(Node);
  if (!Value.empty()) {
    OS << "\n";
    printHtml(OS, Value);
  }
  ...
  for (diff::NodeId const Child : Node.Children)
...
static void printNodeAttributes(raw_ostream &OS, diff::SyntaxTree &Tree,
  ...
  std::string const Value = Tree.getNodeValue(N);
  if (!Value.empty()) {
    OS << R"(,"value":")";
...
static void printNode(raw_ostream &OS, diff::SyntaxTree &Tree,
  ...
  std::string const Value = Tree.getNodeValue(Id);
  if (!Value.empty())
    OS << ": " << Value;
  OS << "(" << Id << ")";
}
...
static void printDstChange(raw_ostream &OS, diff::ASTDiff &Diff,
  ...
  diff::NodeId const Src = Diff.getMapped(DstTree, Dst);
...
int main(int argc, const char **argv) {
  ...
  for (diff::NodeId const Dst : DstTree) {
    diff::NodeId const Src = Diff.getMapped(DstTree, Dst);
  ...
  for (diff::NodeId const Src : SrcTree) {
