...
IncrementalParser::ParseOrWrapTopLevelDecl() {
  ...
  Sema &S = CI->getSema();
  llvm::CrashRecoveryContextCleanupRegistrar<Sema> const CleanupSema(&S);
  ...
  if (Diags.hasErrorOccurred()) {
    ...
    if (StoredDeclsMap *Map = PreviousTU->getLookupPtr()) {
      for (auto I = Map->begin(); I != Map->end(); ++I) {
        ...
        DeclContextLookupResult const R = List.getLookupResult();
        for (NamedDecl *D : R)
...
IncrementalParser::Parse(llvm::StringRef input) {
  ...
  size_t const InputSize = input.size(); // don't include trailing 0
  ...
  SourceLocation const NewLoc = SM.getLocForStartOfFile(SM.getMainFileID());

  // Create FileID for the current buffer.
  FileID const FID = SM.createFileID(std::move(MB), SrcMgr::C_User, /*LoadedID=*/0,
