...
TEST_F(StencilTest, MaybeDerefSmartPointerFromMemberExpr) {
  StringRef const Id = "id";
  std::string const Snippet = "Smart x; x->Field;";
  auto StmtMatch =
...
TEST_F(StencilTest, DescribeType) {
  std::string const Snippet = "int *x; x;";
  std::string const Expected = "int *";
  auto StmtMatch =
...
TEST_F(StencilTest, DescribeSugaredType) {
  std::string const Snippet = "using Ty = int; Ty *x; x;";
  std::string const Expected = "Ty *";
  auto StmtMatch =
      matchStmt(Snippet, declRefExpr(hasType(qualType().bind("type"))));
  ASSERT_TRUE(StmtMatch);
  EXPECT_THAT_EXPECTED(describe("type")->eval(StmtMatch->Result),
...
TEST_F(StencilTest, DescribeDeclType) {
  std::string const Snippet = "S s; s;";
...
TEST_F(StencilTest, DescribeQualifiedType) {
  std::string const Snippet = "N::C c; c;";
  std::string const Expected = "N::C";
  auto StmtMatch =
      matchStmt(Snippet, declRefExpr(hasType(qualType().bind("type"))));
  ASSERT_TRUE(StmtMatch);
  EXPECT_THAT_EXPECTED(describe("type")->eval(StmtMatch->Result),
...
TEST_F(StencilTest, DescribeUnqualifiedType) {
  std::string const Snippet = "using N::C; C c; c;";
  std::string const Expected = "N::C";
  auto StmtMatch =
      matchStmt(Snippet, declRefExpr(hasType(qualType().bind("type"))));
  ASSERT_TRUE(StmtMatch);
...
TEST_F(StencilTest, DescribeAnonNamespaceType) {
  std::string const Snippet = "AnonC c; c;";
  std::string const Expected = "(anonymous namespace)::AnonC";
  auto StmtMatch =
      matchStmt(Snippet, declRefExpr(hasType(qualType().bind("type"))));
  ASSERT_TRUE(StmtMatch);
  EXPECT_THAT_EXPECTED(describe("type")->eval(StmtMatch->Result),
...
TEST(StencilToStringTest, RawTextOp) {
  ...
  StringRef const Expected = R"("foo bar baz")";
...
TEST(StencilToStringTest, DescribeOp) {
  auto S = describe("Id");
  StringRef const Expected = R"repr(describe("Id"))repr";
  EXPECT_EQ(S->toString(), Expected);
}

TEST(StencilToStringTest, DebugPrintNodeOp) {
  auto S = dPrint("Id");
  StringRef const Expected = R"repr(dPrint("Id"))repr";
  EXPECT_EQ(S->toString(), Expected);
}

TEST(StencilToStringTest, ExpressionOp) {
  auto S = expression("Id");
  StringRef const Expected = R"repr(expression("Id"))repr";
  EXPECT_EQ(S->toString(), Expected);
}

TEST(StencilToStringTest, DerefOp) {
  auto S = deref("Id");
  StringRef const Expected = R"repr(deref("Id"))repr";
  EXPECT_EQ(S->toString(), Expected);
}

TEST(StencilToStringTest, AddressOfOp) {
  auto S = addressOf("Id");
  StringRef const Expected = R"repr(addressOf("Id"))repr";
  EXPECT_EQ(S->toString(), Expected);
}
...
TEST(StencilToStringTest, Sequence) {
  ...
  StringRef const Expected = R"repr(seq("foo", access("x", "m()"), "bar", )repr"
...
TEST(StencilToStringTest, SequenceEmpty) {
  auto S = cat();
  StringRef const Expected = "seq()";
  EXPECT_EQ(S->toString(), Expected);
}

TEST(StencilToStringTest, SequenceSingle) {
  auto S = cat("foo");
  StringRef const Expected = "\"foo\"";
  EXPECT_EQ(S->toString(), Expected);
}
