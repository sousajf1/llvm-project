...
template <typename M> TestMatch matchCode(StringRef Code, M Matcher) {
  ...
  TraversalKindScope const RAII(Context, TK_AsIs);
...
TEST(RangeSelectorTest, BeforeOp) {
  ...
  StringRef const CallID = "call";
  ast_matchers::internal::Matcher<Stmt> const M = callExpr().bind(CallID);
  RangeSelector const R = before(node(CallID.str()));

  TestMatch const Match = matchCode(Code, M);
...
TEST(RangeSelectorTest, AfterOp) {
  ...
  StringRef const Call = "call";
  TestMatch const Match = matchCode(Code, callExpr().bind(Call));
...
#define ISNULL(x) x == nullptr
  ...
  int const YVarLen = 1;
  SourceLocation const After = getSpellingLocAfter(Match.Result, "yvar", YVarLen);
  CharSourceRange const Expected = CharSourceRange::getCharRange(After, After);
...
TEST(RangeSelectorTest, BetweenOp) {
  ...
  RangeSelector const R = between(node("a0"), node("a1"));
  TestMatch const Match = matchCode(Code, Matcher);
...
TEST(RangeSelectorTest, EncloseOpNodes) {
  ...
  auto Matcher = callExpr(hasArgument(0, expr().bind("a0")),
                          hasArgument(1, expr().bind("a1")));
  RangeSelector const R = encloseNodes("a0", "a1");
  TestMatch const Match = matchCode(Code, Matcher);
...
TEST(RangeSelectorTest, EncloseOpGeneral) {
  ...
  auto Matcher = callExpr(hasArgument(0, expr().bind("a0")),
                          hasArgument(1, expr().bind("a1")));
  RangeSelector const R = enclose(node("a0"), node("a1"));
  TestMatch const Match = matchCode(Code, Matcher);
  EXPECT_THAT_EXPECTED(select(R, Match), HasValue("3, 7"));
}
...
TEST(RangeSelectorTest, NodeOpStatement) {
  StringRef const Code = "int f() { return 3; }";
  TestMatch const Match = matchCode(Code, returnStmt().bind("id"));
  EXPECT_THAT_EXPECTED(select(node("id"), Match), HasValue("return 3;"));
}

TEST(RangeSelectorTest, NodeOpExpression) {
  StringRef const Code = "int f() { return 3; }";
  TestMatch const Match = matchCode(Code, expr().bind("id"));
...
TEST(RangeSelectorTest, StatementOp) {
  StringRef const Code = "int f() { return 3; }";
  TestMatch const Match = matchCode(Code, expr().bind("id"));
  RangeSelector const R = statement("id");
...
TEST(RangeSelectorTest, MemberOp) {
  ...
  const char *ID = "id";
  TestMatch const Match = matchCode(Code, memberExpr().bind(ID));
...
TEST(RangeSelectorTest, NameOpNamedDecl) {
  ...
  const char *ID = "id";
  TestMatch const Match = matchCode(Code, functionDecl().bind(ID));
...
TEST(RangeSelectorTest, NameOpDeclRef) {
  ...
  const char *Ref = "ref";
  TestMatch const Match = matchCode(Code, declRefExpr(to(functionDecl())).bind(Ref));
...
TEST(RangeSelectorTest, NameOpCtorInitializer) {
  ...
  TestMatch const Match = matchCode(Code, cxxCtorInitializer().bind(Init));
...
TEST(RangeSelectorTest, NameOpTypeLoc) {
  ...
  TestMatch const MatchA = matchCode(
  ...
  TestMatch const MatchB = matchCode(
  ...
  TestMatch const MatchC = matchCode(
...
TEST(RangeSelectorTest, CallArgsOp) {
  ...
  const char *ID = "id";
  TestMatch const Match = matchCode(Code, callExpr().bind(ID));
...
TEST(RangeSelectorTest, CallArgsOpEolComments) {
  ...
  const char *ID = "id";
  TestMatch const Match = matchCode(Code, callExpr().bind(ID));
  std::string const ExpectedString = R"(  // Header
          1,           // foo
          2            // bar
      )";
...
TEST(RangeSelectorTest, StatementsOp) {
  ...
  const char *ID = "id";
  TestMatch const Match = matchCode(Code, compoundStmt().bind(ID));
  EXPECT_THAT_EXPECTED(
...
TEST(RangeSelectorTest, ElementsOp) {
  ...
  const char *ID = "id";
  TestMatch const Match = matchCode(Code, initListExpr().bind(ID));
  EXPECT_THAT_EXPECTED(
...
TEST(RangeSelectorTest, ElseBranchOpSingleStatement) {
  ...
  const char *ID = "id";
  TestMatch const Match = matchCode(Code, ifStmt().bind(ID));
...
#define BADDECL(E) int bad(int x) { return E; }
  ...
  const char *Fun = "Fun";
  TestMatch const Match = matchCode(Code, functionDecl(hasName("bad")).bind(Fun));
...
#define BADDECL(E) int bad(int x) { return E; }
  ...
  const char *Ret = "Ret";
  TestMatch const Match = matchCode(Code, returnStmt().bind(Ret));
...
TEST(RangeSelectorTest, IfBoundOpUnbound) {
  ...
  const char *ID = "id", *Op = "op";
  TestMatch const Match = matchCode(Code, binaryOperator().bind(Op));
  EXPECT_THAT_EXPECTED(select(ifBound(ID, node(ID), node(Op)), Match),
                       HasValue("3 + 5"));
}
