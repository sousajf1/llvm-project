...
struct BaseDefiningValueResult {
  ...
  BaseDefiningValueResult(Value *BDV, bool IsKnownBase)
    ...
    // a base.
    bool const MustBeBase = isKnownBaseResult(BDV);
...
static Value *findBasePointer(Value *I, DefiningValueMapTy &Cache) {
  ...
  while (Progress) {
    ...
    for (auto Pair : States) {
      ...
      BDVState const OldState = States[BDV];
  ...
  for (auto Pair : States) {
    ...
    BDVState const State = Pair.second;
...
static AttributeList legalizeCallAttributes(LLVMContext &Ctx,
  ...
  for (Attribute const A : AL.getFnAttrs()) {
...
static void CreateGCRelocates(ArrayRef<Value *> LiveVariables,
  ...
  auto FindIndex = [](ArrayRef<Value *> LiveVec, Value *Val) {
    ...
    size_t const Index = std::distance(LiveVec.begin(), ValIt);
...
makeStatepointExplicitImpl(CallBase *Call, /* to replace */
  ...
  ArrayRef<Value *> const GCArgs(LiveVariables);
  ...
  StringRef const DeoptLowering = getDeoptLowering(Call);
  ...
  if (Function *F = dyn_cast<Function>(CallTarget)) {
    ...
    if (IID == Intrinsic::experimental_deoptimize) {
      ...
      auto GetBaseAndOffset = [&](Value *Derived) {
        ...
        unsigned const AddressSpace = Derived->getType()->getPointerAddressSpace();
        unsigned const IntPtrSize = DL.getPointerSizeInBits(AddressSpace);
      ...
      auto GetFunctionName = [](Intrinsic::ID IID, ConstantInt *ElementSizeCI) {
        uint64_t const ElementSize = ElementSizeCI->getZExtValue();
  ...
  if (IsDeoptimize) {
    ...
    if (!Call->getType()->isVoidTy() && !Call->use_empty()) {
      StringRef const Name = Call->hasName() ? Call->getName() : "";
...
static void relocationViaAlloca(
  ...
  for (Instruction  const&I : F.getEntryBlock())
    if (isa<AllocaInst>(I))
      InitialAllocaNum++;
#endif
...
static void insertUseHolderAfter(CallBase *Call, const ArrayRef<Value *> Values,
  ...
  FunctionCallee const Func = M->getOrInsertFunction(
...
static bool AreEquivalentPhiNodes(PHINode &OrigRootPhi, PHINode &AlternateRootPhi) {
  unsigned const PhiNum = OrigRootPhi.getNumIncomingValues();
...
static bool insertParsePoints(Function &F, DominatorTree &DT,
  ...
  for (size_t i = 0; i < Records.size(); i++) {
    PartiallyConstructedSafepointRecord  const&Info = Records[i];
...
static void stripInvalidMetadataFromInstruction(Instruction &I) {
  ...
  unsigned const ValidMetadataAfterRS4GC[] = {LLVMContext::MD_tbaa,
