...
SDValue DAGTypeLegalizer::PromoteIntRes_MERGE_VALUES(SDNode *N,
                                                     unsigned ResNo) {
  SDValue const Op = DisintegrateMERGE_VALUES(N, ResNo);
  return GetPromotedInteger(Op);
}

SDValue DAGTypeLegalizer::PromoteIntRes_AssertSext(SDNode *N) {
  // Sign-extend the new bits, and continue the assertion.
  SDValue const Op = SExtPromotedInteger(N->getOperand(0));
...
SDValue DAGTypeLegalizer::PromoteIntRes_AssertZext(SDNode *N) {
  ...
  SDValue const Op = ZExtPromotedInteger(N->getOperand(0));
  return DAG.getNode(ISD::AssertZext, SDLoc(N),
                     Op.getValueType(), Op, N->getOperand(1));
}

SDValue DAGTypeLegalizer::PromoteIntRes_Atomic0(AtomicSDNode *N) {
  EVT const ResVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
...
SDValue DAGTypeLegalizer::PromoteIntRes_Atomic1(AtomicSDNode *N) {
  SDValue const Op2 = GetPromotedInteger(N->getOperand(2));
  SDValue Res = DAG.getAtomic(N->getOpcode(), SDLoc(N),
                              N->getMemoryVT(),
                              N->getChain(), N->getBasePtr(),
                              Op2, N->getMemOperand());
...
SDValue DAGTypeLegalizer::PromoteIntRes_AtomicCmpSwap(AtomicSDNode *N,
  ...
  if (ResNo == 1) {
    ...
    EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(1));
    ...
    SDVTList const VTs = DAG.getVTList(N->getValueType(0), SVT, MVT::Other);
    SDValue const Res = DAG.getAtomicCmpSwap(
  ...
  SDValue const Op3 = GetPromotedInteger(N->getOperand(3));
...
SDValue DAGTypeLegalizer::PromoteIntRes_BITCAST(SDNode *N) {
  ...
  EVT const NInVT = TLI.getTypeToTransformTo(*DAG.getContext(), InVT);
  EVT const OutVT = N->getValueType(0);
  EVT const NOutVT = TLI.getTypeToTransformTo(*DAG.getContext(), OutVT);
  SDLoc const dl(N);
  ...
  case TargetLowering::TypeWidenVector:
    ...
    if (NOutVT.bitsEq(NInVT) && !NOutVT.isVector()) {
      ...
      if (DAG.getDataLayout().isBigEndian()) {
        unsigned const ShiftAmt = NInVT.getSizeInBits() - InVT.getSizeInBits();
        EVT const ShiftAmtTy = TLI.getShiftAmountTy(NOutVT, DAG.getDataLayout());
    ...
    if (NOutVT.isVector()) {
      unsigned const WidenInSize = NInVT.getSizeInBits();
      unsigned const OutSize = OutVT.getSizeInBits();
      if (WidenInSize % OutSize == 0) {
        unsigned const Scale = WidenInSize / OutSize;
        EVT const WideOutVT = EVT::getVectorVT(*DAG.getContext(),
...
SDValue DAGTypeLegalizer::PromoteIntRes_FREEZE(SDNode *N) {
  SDValue const V = GetPromotedInteger(N->getOperand(0));
  return DAG.getNode(ISD::FREEZE, SDLoc(N),
                     V.getValueType(), V);
}

SDValue DAGTypeLegalizer::PromoteIntRes_BSWAP(SDNode *N) {
  SDValue const Op = GetPromotedInteger(N->getOperand(0));
  EVT const OVT = N->getValueType(0);
  EVT const NVT = Op.getValueType();
  SDLoc const dl(N);
  ...
  if (!OVT.isVector() &&
    ...
    if (SDValue const Res = TLI.expandBSWAP(N, DAG))
      return DAG.getNode(ISD::ANY_EXTEND, dl, NVT, Res);
  }

  unsigned const DiffBits = NVT.getScalarSizeInBits() - OVT.getScalarSizeInBits();
  EVT const ShiftVT = getShiftAmountTyForConstant(NVT, TLI, DAG);
...
SDValue DAGTypeLegalizer::PromoteIntRes_BITREVERSE(SDNode *N) {
  ...
  if (!OVT.isVector() && OVT.isSimple() &&
    ...
    if (SDValue const Res = TLI.expandBITREVERSE(N, DAG))
      return DAG.getNode(ISD::ANY_EXTEND, dl, NVT, Res);
  }
...
SDValue DAGTypeLegalizer::PromoteIntRes_Constant(SDNode *N) {
  ...
  unsigned const Opc = VT.isByteSized() ? ISD::SIGN_EXTEND : ISD::ZERO_EXTEND;
...
SDValue DAGTypeLegalizer::PromoteIntRes_EXTRACT_VECTOR_ELT(SDNode *N) {
  SDLoc const dl(N);
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  ...
  if (TLI.getTypeAction(*DAG.getContext(), Op0.getValueType())
    ...
    SDValue const In = GetPromotedInteger(Op0);

    // If the new type is larger than NVT, use it. We probably won't need to
    // promote it again.
    EVT const SVT = In.getValueType().getScalarType();
    if (SVT.bitsGE(NVT)) {
      SDValue const Ext = DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl, SVT, In, Op1);
...
SDValue DAGTypeLegalizer::PromoteIntRes_ISNAN(SDNode *N) {
  SDLoc const DL(N);
  EVT const ResultVT = N->getValueType(0);
  EVT const NewResultVT = TLI.getTypeToTransformTo(*DAG.getContext(), ResultVT);
...
SDValue DAGTypeLegalizer::PromoteIntRes_MLOAD(MaskedLoadSDNode *N) {
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  SDValue const ExtPassThru = GetPromotedInteger(N->getPassThru());
...
SDValue DAGTypeLegalizer::PromoteIntRes_MGATHER(MaskedGatherSDNode *N) {
  ...
  SDLoc const dl(N);
  SDValue const Ops[] = {N->getChain(), ExtPassThru, N->getMask(), N->getBasePtr(),
...
SDValue DAGTypeLegalizer::PromoteIntRes_Overflow(SDNode *N) {
  ...
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(1));
  EVT const VT = N->getValueType(0);
  EVT const SVT = getSetCCResultType(VT);
  ...
  SDLoc const dl(N);
  SDValue const Res = DAG.getNode(N->getOpcode(), dl, DAG.getVTList(VT, SVT),
...
SDValue DAGTypeLegalizer::PromoteIntRes_ADDSUBSHLSAT(SDNode *N) {
  ...
  SDLoc const dl(N);
  SDValue const Op1 = N->getOperand(0);
  SDValue const Op2 = N->getOperand(1);
  unsigned const OldBits = Op1.getScalarValueSizeInBits();

  unsigned const Opcode = N->getOpcode();
  bool const IsShift = Opcode == ISD::USHLSAT || Opcode == ISD::SSHLSAT;
  ...
  EVT const PromotedType = Op1Promoted.getValueType();
  unsigned const NewBits = PromotedType.getScalarSizeInBits();

  if (Opcode == ISD::UADDSAT) {
    APInt const MaxVal = APInt::getAllOnesValue(OldBits).zext(NewBits);
    SDValue const SatMax = DAG.getConstant(MaxVal, dl, PromotedType);
    SDValue const Add =
  ...
  if (IsShift || TLI.isOperationLegalOrCustom(Opcode, PromotedType)) {
    ...
    unsigned const SHLAmount = NewBits - OldBits;
    EVT const SHVT = TLI.getShiftAmountTy(PromotedType, DAG.getDataLayout());
    SDValue const ShiftAmount = DAG.getConstant(SHLAmount, dl, SHVT);
    Op1Promoted =
  ...
  unsigned const AddOp = Opcode == ISD::SADDSAT ? ISD::ADD : ISD::SUB;
  APInt const MinVal = APInt::getSignedMinValue(OldBits).sext(NewBits);
  APInt const MaxVal = APInt::getSignedMaxValue(OldBits).sext(NewBits);
  SDValue const SatMin = DAG.getConstant(MinVal, dl, PromotedType);
  SDValue const SatMax = DAG.getConstant(MaxVal, dl, PromotedType);
  SDValue Result =
...
SDValue DAGTypeLegalizer::PromoteIntRes_MULFIX(SDNode *N) {
  ...
  EVT const OldType = N->getOperand(0).getValueType();
  EVT const PromotedType = Op1Promoted.getValueType();
  unsigned const DiffSize =
  ...
  if (Saturating) {
    ...
    EVT const ShiftTy = TLI.getShiftAmountTy(PromotedType, DAG.getDataLayout());
    Op1Promoted = DAG.getNode(ISD::SHL, dl, PromotedType, Op1Promoted,
                              DAG.getConstant(DiffSize, dl, ShiftTy));
    SDValue const Result = DAG.getNode(N->getOpcode(), dl, PromotedType, Op1Promoted,
                                 Op2Promoted, N->getOperand(2));
    unsigned const ShiftOp = Signed ? ISD::SRA : ISD::SRL;
...
static SDValue SaturateWidenedDIVFIX(SDValue V, SDLoc &dl,
  ...
  unsigned const VTW = VT.getScalarSizeInBits();
...
static SDValue earlyExpandDIVFIX(SDNode *N, SDValue LHS, SDValue RHS,
  ...
  bool const Signed = N->getOpcode() == ISD::SDIVFIX ||
                N->getOpcode() == ISD::SDIVFIXSAT;
  bool const Saturating = N->getOpcode() == ISD::SDIVFIXSAT ||
...
SDValue DAGTypeLegalizer::PromoteIntRes_DIVFIX(SDNode *N) {
  ...
  EVT const PromotedType = Op1Promoted.getValueType();
  unsigned const Scale = N->getConstantOperandVal(2);
  ...
  if (TLI.isTypeLegal(PromotedType)) {
    TargetLowering::LegalizeAction const Action =
    ...
    if (Action == TargetLowering::Legal || Action == TargetLowering::Custom) {
      EVT const ShiftTy = TLI.getShiftAmountTy(PromotedType, DAG.getDataLayout());
      unsigned const Diff = PromotedType.getScalarSizeInBits() -
...
SDValue DAGTypeLegalizer::PromoteIntRes_SADDSUBO(SDNode *N, unsigned ResNo) {
  ...
  SDValue const LHS = SExtPromotedInteger(N->getOperand(0));
  SDValue const RHS = SExtPromotedInteger(N->getOperand(1));
  EVT const OVT = N->getOperand(0).getValueType();
  EVT const NVT = LHS.getValueType();
  SDLoc const dl(N);

  // Do the arithmetic in the larger type.
  unsigned const Opcode = N->getOpcode() == ISD::SADDO ? ISD::ADD : ISD::SUB;
...
SDValue DAGTypeLegalizer::PromoteIntRes_SELECT(SDNode *N) {
  SDValue const LHS = GetPromotedInteger(N->getOperand(1));
  SDValue const RHS = GetPromotedInteger(N->getOperand(2));
  return DAG.getSelect(SDLoc(N),
                       LHS.getValueType(), N->getOperand(0), LHS, RHS);
}

SDValue DAGTypeLegalizer::PromoteIntRes_VSELECT(SDNode *N) {
  SDValue const Mask = N->getOperand(0);
...
SDValue DAGTypeLegalizer::PromoteIntRes_SELECT_CC(SDNode *N) {
  SDValue const LHS = GetPromotedInteger(N->getOperand(2));
  SDValue const RHS = GetPromotedInteger(N->getOperand(3));
...
SDValue DAGTypeLegalizer::PromoteIntRes_SETCC(SDNode *N) {
  ...
  SDValue SetCC;
  if (N->isStrictFPOpcode()) {
    EVT const VTs[] = {SVT, MVT::Other};
    SDValue const Opers[] = {N->getOperand(0), N->getOperand(1),
...
SDValue DAGTypeLegalizer::PromoteIntRes_SHL(SDNode *N) {
  SDValue const LHS = GetPromotedInteger(N->getOperand(0));
...
SDValue DAGTypeLegalizer::PromoteIntRes_SimpleIntBinOp(SDNode *N) {
  ...
  SDValue const LHS = GetPromotedInteger(N->getOperand(0));
  SDValue const RHS = GetPromotedInteger(N->getOperand(1));
...
SDValue DAGTypeLegalizer::PromoteIntRes_ZExtIntBinOp(SDNode *N) {
  ...
  SDValue const LHS = ZExtPromotedInteger(N->getOperand(0));
  SDValue const RHS = ZExtPromotedInteger(N->getOperand(1));
  return DAG.getNode(N->getOpcode(), SDLoc(N),
                     LHS.getValueType(), LHS, RHS);
}
...
SDValue DAGTypeLegalizer::PromoteIntRes_UMINUMAX(SDNode *N) {
  ...
  SDValue const LHS = SExtOrZExtPromotedInteger(N->getOperand(0));
  SDValue const RHS = SExtOrZExtPromotedInteger(N->getOperand(1));
  return DAG.getNode(N->getOpcode(), SDLoc(N),
                     LHS.getValueType(), LHS, RHS);
}
...
SDValue DAGTypeLegalizer::PromoteIntRes_FunnelShift(SDNode *N) {
  ...
  SDLoc const DL(N);
  EVT const OldVT = N->getOperand(0).getValueType();
  EVT const VT = Lo.getValueType();
  unsigned const Opcode = N->getOpcode();
  bool const IsFSHR = Opcode == ISD::FSHR;
  unsigned const OldBits = OldVT.getScalarSizeInBits();
  unsigned const NewBits = VT.getScalarSizeInBits();
  ...
  if (NewBits >= (2 * OldBits) && !isa<ConstantSDNode>(Amount) &&
    ...
    SDValue const HiShift = DAG.getConstant(OldBits, DL, VT);
  ...
  SDValue const ShiftOffset = DAG.getConstant(NewBits - OldBits, DL, VT);
...
SDValue DAGTypeLegalizer::PromoteIntRes_TRUNCATE(SDNode *N) {
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  SDValue Res;
  SDValue const InOp = N->getOperand(0);
  SDLoc const dl(N);
  ...
  case TargetLowering::TypeSplitVector: {
    ...
    ElementCount const NumElts = InVT.getVectorElementCount();
    ...
    EVT const HalfNVT = EVT::getVectorVT(*DAG.getContext(), NVT.getScalarType(),
  ...
  case TargetLowering::TypeWidenVector: {
    SDValue const WideInOp = GetWidenedVector(InOp);

    // Truncate widened InOp.
    unsigned const NumElem = WideInOp.getValueType().getVectorNumElements();
    EVT const TruncVT = EVT::getVectorVT(*DAG.getContext(),
                                   N->getValueType(0).getScalarType(), NumElem);
    SDValue const WideTrunc = DAG.getNode(ISD::TRUNCATE, dl, TruncVT, WideInOp);

    // Zero extend so that the elements are of same type as those of NVT
    EVT const ExtVT = EVT::getVectorVT(*DAG.getContext(), NVT.getVectorElementType(),
                                 NumElem);
    SDValue const WideExt = DAG.getNode(ISD::ZERO_EXTEND, dl, ExtVT, WideTrunc);
...
SDValue DAGTypeLegalizer::PromoteIntRes_UADDSUBO(SDNode *N, unsigned ResNo) {
  ...
  // Do the arithmetic in the larger type.
  unsigned const Opcode = N->getOpcode() == ISD::UADDO ? ISD::ADD : ISD::SUB;
  SDValue Res = DAG.getNode(Opcode, dl, NVT, LHS, RHS);
...
SDValue DAGTypeLegalizer::PromoteIntRes_ADDSUBCARRY(SDNode *N, unsigned ResNo) {
  ...
  EVT const ValueVTs[] = {LHS.getValueType(), N->getValueType(1)};

  // Do the arithmetic in the wide type.
  SDValue const Res = DAG.getNode(N->getOpcode(), SDLoc(N), DAG.getVTList(ValueVTs),
...
SDValue DAGTypeLegalizer::PromoteIntRes_ABS(SDNode *N) {
  SDValue const Op0 = SExtPromotedInteger(N->getOperand(0));
...
SDValue DAGTypeLegalizer::PromoteIntRes_XMULO(SDNode *N, unsigned ResNo) {
  ...
  SDLoc const DL(N);
  EVT const SmallVT = LHS.getValueType();
  ...
  SDVTList const VTs = DAG.getVTList(LHS.getValueType(), N->getValueType(1));
  ...
  if (N->getOpcode() == ISD::UMULO) {
    ...
    unsigned const Shift = SmallVT.getScalarSizeInBits();
    EVT const ShiftTy = getShiftAmountTyForConstant(Mul.getValueType(), TLI, DAG);
    SDValue const Hi = DAG.getNode(ISD::SRL, DL, Mul.getValueType(), Mul,
    ...
    SDValue const SExt = DAG.getNode(ISD::SIGN_EXTEND_INREG, DL, Mul.getValueType(),
...
SDValue DAGTypeLegalizer::PromoteIntRes_VSCALE(SDNode *N) {
  EVT const VT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));

  APInt const MulImm = cast<ConstantSDNode>(N->getOperand(0))->getAPIntValue();
  return DAG.getVScale(SDLoc(N), VT, MulImm.sextOrSelf(VT.getSizeInBits()));
}

SDValue DAGTypeLegalizer::PromoteIntRes_VAARG(SDNode *N) {
  SDValue Chain = N->getOperand(0); // Get the chain.
  SDValue const Ptr = N->getOperand(1); // Get the pointer.
  EVT const VT = N->getValueType(0);
  SDLoc const dl(N);

  MVT const RegVT = TLI.getRegisterType(*DAG.getContext(), VT);
  unsigned const NumRegs = TLI.getNumRegisters(*DAG.getContext(), VT);
...
void DAGTypeLegalizer::PromoteSetCCOperands(SDValue &NewLHS,SDValue &NewRHS,
  ...
  case ISD::SETEQ:
  case ISD::SETNE: {
    SDValue const OpL = GetPromotedInteger(NewLHS);
    SDValue const OpR = GetPromotedInteger(NewRHS);
    ...
    unsigned const OpLEffectiveBits =
        OpL.getScalarValueSizeInBits() - DAG.ComputeNumSignBits(OpL) + 1;
    unsigned const OpREffectiveBits =
...
SDValue DAGTypeLegalizer::PromoteIntOp_BRCOND(SDNode *N, unsigned OpNo) {
  ...
  SDValue const Cond = PromoteTargetBoolean(N->getOperand(1), MVT::Other);
...
SDValue DAGTypeLegalizer::PromoteIntOp_BUILD_PAIR(SDNode *N) {
  ...
  EVT const OVT = N->getOperand(0).getValueType();
  SDValue const Lo = ZExtPromotedInteger(N->getOperand(0));
...
SDValue DAGTypeLegalizer::PromoteIntOp_INSERT_VECTOR_ELT(SDNode *N,
  ...
  // Promote the index.
  SDValue const Idx = DAG.getZExtOrTrunc(N->getOperand(2), SDLoc(N),
...
SDValue DAGTypeLegalizer::PromoteIntOp_SELECT(SDNode *N, unsigned OpNo) {
  ...
  EVT const OpTy = N->getOperand(1).getValueType();

  if (N->getOpcode() == ISD::VSELECT)
    if (SDValue const Res = WidenVSELECTMask(N))
      return DAG.getNode(N->getOpcode(), SDLoc(N), N->getValueType(0),
                         Res, N->getOperand(1), N->getOperand(2));

  // Promote all the way up to the canonical SetCC type.
  EVT const OpVT = N->getOpcode() == ISD::SELECT ? OpTy.getScalarType() : OpTy;
...
SDValue DAGTypeLegalizer::PromoteIntOp_STORE(StoreSDNode *N, unsigned OpNo){
  ...
  SDValue const Val = GetPromotedInteger(N->getValue());  // Get promoted value.
...
SDValue DAGTypeLegalizer::PromoteIntOp_MSTORE(MaskedStoreSDNode *N,
  ...
  if (OpNo == 4) {
    ...
    EVT const DataVT = DataOp.getValueType();
...
SDValue DAGTypeLegalizer::PromoteIntOp_MLOAD(MaskedLoadSDNode *N,
  ...
  EVT const DataVT = N->getValueType(0);
  SDValue const Mask = PromoteTargetBoolean(N->getOperand(OpNo), DataVT);
  SmallVector<SDValue, 4> NewOps(N->op_begin(), N->op_end());
  NewOps[OpNo] = Mask;
...
SDValue DAGTypeLegalizer::PromoteIntOp_MSCATTER(MaskedScatterSDNode *N,
  ...
  if (OpNo == 2) {
    // The Mask
    EVT const DataVT = N->getValue().getValueType();
    NewOps[OpNo] = PromoteTargetBoolean(N->getOperand(OpNo), DataVT);
  } else if (OpNo == 4) {
    // The Index
    if (N->isIndexSigned())
...
SDValue DAGTypeLegalizer::PromoteIntOp_FIX(SDNode *N) {
  SDValue const Op2 = ZExtPromotedInteger(N->getOperand(2));
  return SDValue(
...
SDValue DAGTypeLegalizer::PromoteIntOp_PREFETCH(SDNode *N, unsigned OpNo) {
  ...
  // width.
  SDValue const Op2 = ZExtPromotedInteger(N->getOperand(2));
  SDValue const Op3 = ZExtPromotedInteger(N->getOperand(3));
  SDValue const Op4 = ZExtPromotedInteger(N->getOperand(4));
...
SDValue DAGTypeLegalizer::PromoteIntOp_FPOWI(SDNode *N) {
  ...
  RTLIB::Libcall const LC = RTLIB::getPOWI(N->getValueType(0));
  ...
  SDValue const Ops[2] = { N->getOperand(0), N->getOperand(1) };
  std::pair<SDValue, SDValue> const Tmp =
...
SDValue DAGTypeLegalizer::PromoteIntOp_VECREDUCE(SDNode *N) {
  ...
  SDValue const Reduce = DAG.getNode(N->getOpcode(), dl, EltVT, Op);
  return DAG.getNode(ISD::TRUNCATE, dl, VT, Reduce);
}

SDValue DAGTypeLegalizer::PromoteIntOp_SET_ROUNDING(SDNode *N) {
  SDValue const Op = ZExtPromotedInteger(N->getOperand(1));
...
void DAGTypeLegalizer::ExpandIntegerResult(SDNode *N, unsigned ResNo) {
  ...
  case ISD::ATOMIC_SWAP:
  case ISD::ATOMIC_CMP_SWAP: {
    std::pair<SDValue, SDValue> const Tmp = ExpandAtomic(N);
  ...
  case ISD::ATOMIC_CMP_SWAP_WITH_SUCCESS: {
    ...
    SDVTList const VTs = DAG.getVTList(N->getValueType(0), MVT::Other);
    SDValue const Tmp = DAG.getAtomicCmpSwap(
    ...
    // comparison.
    SDValue const Success = DAG.getSetCC(SDLoc(N), N->getValueType(1), Tmp,
...
std::pair <SDValue, SDValue> DAGTypeLegalizer::ExpandAtomic(SDNode *Node) {
  unsigned const Opc = Node->getOpcode();
  MVT const VT = cast<AtomicSDNode>(Node)->getMemoryVT().getSimpleVT();
  AtomicOrdering const order = cast<AtomicSDNode>(Node)->getMergedOrdering();
  ...
  EVT const RetVT = Node->getValueType(0);
  TargetLowering::MakeLibCallOptions const CallOptions;
...
void DAGTypeLegalizer::ExpandShiftByConstant(SDNode *N, const APInt &Amt,
  ...
  EVT const NVT = InL.getValueType();
  unsigned const VTBits = N->getValueType(0).getSizeInBits();
  unsigned const NVTBits = NVT.getSizeInBits();
  EVT const ShTy = N->getOperand(1).getValueType();
...
ExpandShiftWithKnownAmountBit(SDNode *N, SDValue &Lo, SDValue &Hi) {
  ...
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  EVT const ShTy = Amt.getValueType();
  unsigned const ShBits = ShTy.getScalarSizeInBits();
  unsigned const NVTBits = NVT.getScalarSizeInBits();
  assert(isPowerOf2_32(NVTBits) &&
         "Expanded integer type size not a power of two!");
  SDLoc const dl(N);

  APInt const HighBitMask = APInt::getHighBitsSet(ShBits, ShBits - Log2_32(NVTBits));
  KnownBits const Known = DAG.computeKnownBits(N->getOperand(1));
  ...
  if (HighBitMask.isSubsetOf(Known.Zero)) {
    ...
    // than 32.
    SDValue const Amt2 = DAG.getNode(ISD::XOR, dl, ShTy, Amt,
                               DAG.getConstant(NVTBits - 1, dl, ShTy));
    ...
    // shift by one bit.
    SDValue const Sh1 = DAG.getNode(Op2, dl, NVT, InL, DAG.getConstant(1, dl, ShTy));
    // Then compute the remaining shift with amount-1.
    SDValue const Sh2 = DAG.getNode(Op2, dl, NVT, Sh1, Amt2);
...
ExpandShiftWithUnknownAmountBit(SDNode *N, SDValue &Lo, SDValue &Hi) {
  SDValue const Amt = N->getOperand(1);
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  EVT const ShTy = Amt.getValueType();
  unsigned const NVTBits = NVT.getSizeInBits();
  assert(isPowerOf2_32(NVTBits) &&
  ...
  SDValue const NVBitsNode = DAG.getConstant(NVTBits, dl, ShTy);
  SDValue const AmtExcess = DAG.getNode(ISD::SUB, dl, ShTy, Amt, NVBitsNode);
  SDValue const AmtLack = DAG.getNode(ISD::SUB, dl, ShTy, NVBitsNode, Amt);
  SDValue const isShort = DAG.getSetCC(dl, getSetCCResultType(ShTy),
                                 Amt, NVBitsNode, ISD::SETULT);
  SDValue const isZero = DAG.getSetCC(dl, getSetCCResultType(ShTy),
...
void DAGTypeLegalizer::ExpandIntRes_MINMAX(SDNode *N,
  ...
  // Value types
  EVT const NVT = LHSL.getValueType();
  EVT const CCT = getSetCCResultType(NVT);
  ...
  SDValue const IsHiLeft = DAG.getSetCC(DL, CCT, LHSH, RHSH, CondC);
  SDValue const IsHiEq = DAG.getSetCC(DL, CCT, LHSH, RHSH, ISD::SETEQ);

  // Lo part corresponding to the 'winning' Hi part
  SDValue const LoCmp = DAG.getSelect(DL, NVT, IsHiLeft, LHSL, RHSL);

  // Recursed Lo part if Hi parts are equal, this uses unsigned version
  SDValue const LoMinMax = DAG.getNode(LoOpc, DL, NVT, {LHSL, RHSL});
...
void DAGTypeLegalizer::ExpandIntRes_ADDSUB(SDNode *N,
  ...
  EVT const NVT = LHSL.getValueType();
  SDValue const LoOps[2] = { LHSL, RHSL };
  SDValue HiOps[3] = { LHSH, RHSH };

  bool const HasOpCarry = TLI.isOperationLegalOrCustom(
      N->getOpcode() == ISD::ADD ? ISD::ADDCARRY : ISD::SUBCARRY,
      TLI.getTypeToExpandTo(*DAG.getContext(), NVT));
  if (HasOpCarry) {
    SDVTList const VTList = DAG.getVTList(NVT, getSetCCResultType(NVT));
  ...
  if (hasCarry) {
    SDVTList const VTList = DAG.getVTList(NVT, MVT::Glue);
    if (N->getOpcode() == ISD::ADD) {
  ...
  bool const hasOVF =
    TLI.isOperationLegalOrCustom(N->getOpcode() == ISD::ADD ?
                                   ISD::UADDO : ISD::USUBO,
                                 TLI.getTypeToExpandTo(*DAG.getContext(), NVT));
  TargetLoweringBase::BooleanContent const BoolType = TLI.getBooleanContents(NVT);

  if (hasOVF) {
    EVT const OvfVT = getSetCCResultType(NVT);
    SDVTList const VTList = DAG.getVTList(NVT, OvfVT);
    int RevOpc;
    if (N->getOpcode() == ISD::ADD) {
      RevOpc = ISD::SUB;
      Lo = DAG.getNode(ISD::UADDO, dl, VTList, LoOps);
  ...
  if (N->getOpcode() == ISD::ADD) {
    ...
    Hi = DAG.getNode(ISD::ADD, dl, NVT, makeArrayRef(HiOps, 2));
    SDValue const Cmp1 = DAG.getSetCC(dl, getSetCCResultType(NVT), Lo, LoOps[0],
                                ISD::SETULT);

    if (BoolType == TargetLoweringBase::ZeroOrOneBooleanContent) {
      SDValue const Carry = DAG.getZExtOrTrunc(Cmp1, dl, NVT);
      Hi = DAG.getNode(ISD::ADD, dl, NVT, Hi, Carry);
      return;
    }

    SDValue const Carry1 = DAG.getSelect(dl, NVT, Cmp1,
                                   DAG.getConstant(1, dl, NVT),
                                   DAG.getConstant(0, dl, NVT));
    SDValue const Cmp2 = DAG.getSetCC(dl, getSetCCResultType(NVT), Lo, LoOps[1],
                                ISD::SETULT);
    SDValue const Carry2 = DAG.getSelect(dl, NVT, Cmp2,
...
void DAGTypeLegalizer::ExpandIntRes_ADDSUBC(SDNode *N,
  ...
  GetExpandedInteger(N->getOperand(0), LHSL, LHSH);
  GetExpandedInteger(N->getOperand(1), RHSL, RHSH);
  SDVTList const VTList = DAG.getVTList(LHSL.getValueType(), MVT::Glue);
  SDValue const LoOps[2] = { LHSL, RHSL };
  SDValue HiOps[3] = { LHSH, RHSH };
...
void DAGTypeLegalizer::ExpandIntRes_ADDSUBE(SDNode *N,
  ...
  GetExpandedInteger(N->getOperand(1), RHSL, RHSH);
  SDVTList const VTList = DAG.getVTList(LHSL.getValueType(), MVT::Glue);
  SDValue const LoOps[3] = { LHSL, RHSL, N->getOperand(2) };
  SDValue HiOps[3] = { LHSH, RHSH };
...
void DAGTypeLegalizer::ExpandIntRes_UADDSUBO(SDNode *N,
  ...
  bool const HasCarryOp = TLI.isOperationLegalOrCustom(
  ...
  if (HasCarryOp) {
    ...
    SDVTList const VTList = DAG.getVTList(LHSL.getValueType(), N->getValueType(1));
    SDValue const LoOps[2] = { LHSL, RHSL };
    SDValue HiOps[3] = { LHSH, RHSH };
    ...
    SDValue const Sum = DAG.getNode(NoCarryOp, dl, LHS.getValueType(), LHS, RHS);
...
void DAGTypeLegalizer::ExpandIntRes_SADDSUBO_CARRY(SDNode *N,
  ...
  unsigned const CarryOp = N->getOpcode() == ISD::SADDO_CARRY ? ISD::ADDCARRY
                                                        : ISD::SUBCARRY;
...
void DAGTypeLegalizer::ExpandIntRes_ANY_EXTEND(SDNode *N,
  ...
  if (Op.getValueType().bitsLE(NVT)) {
    ...
    SDValue const Res = GetPromotedInteger(Op);
    assert(Res.getValueType() == N->getValueType(0) &&
...
void DAGTypeLegalizer::ExpandIntRes_AssertSext(SDNode *N,
  ...
  EVT const NVT = Lo.getValueType();
  EVT const EVT = cast<VTSDNode>(N->getOperand(1))->getVT();
  unsigned const NVTBits = NVT.getSizeInBits();
  unsigned const EVTBits = EVT.getSizeInBits();
...
void DAGTypeLegalizer::ExpandIntRes_Constant(SDNode *N,
                                             SDValue &Lo, SDValue &Hi) {
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  unsigned const NBitWidth = NVT.getSizeInBits();
  auto Constant = cast<ConstantSDNode>(N);
  const APInt &Cst = Constant->getAPIntValue();
  bool const IsTarget = Constant->isTargetOpcode();
  bool const IsOpaque = Constant->isOpaque();
  SDLoc const dl(N);
...
void DAGTypeLegalizer::ExpandIntRes_ABS(SDNode *N, SDValue &Lo, SDValue &Hi) {
  ...
  bool const HasAddCarry = TLI.isOperationLegalOrCustom(
      ISD::ADDCARRY, TLI.getTypeToExpandTo(*DAG.getContext(), NVT));
  if (HasAddCarry) {
    EVT const ShiftAmtTy = getShiftAmountTyForConstant(NVT, TLI, DAG);
    SDValue const Sign =
  ...
  EVT const VT = N->getValueType(0);
  SDValue const Neg = DAG.getNode(ISD::SUB, dl, VT,
                            DAG.getConstant(0, dl, VT), N0);
  SDValue NegLo, NegHi;
  SplitInteger(Neg, NegLo, NegHi);

  SDValue const HiIsNeg = DAG.getSetCC(dl, getSetCCResultType(NVT),
...
void DAGTypeLegalizer::ExpandIntRes_CTLZ(SDNode *N,
  ...
  SDValue const HiNotZero = DAG.getSetCC(dl, getSetCCResultType(NVT), Hi,
                                   DAG.getConstant(0, dl, NVT), ISD::SETNE);

  SDValue const LoLZ = DAG.getNode(N->getOpcode(), dl, NVT, Lo);
  SDValue const HiLZ = DAG.getNode(ISD::CTLZ_ZERO_UNDEF, dl, NVT, Hi);
...
void DAGTypeLegalizer::ExpandIntRes_CTTZ(SDNode *N,
  ...
  SDValue const LoNotZero = DAG.getSetCC(dl, getSetCCResultType(NVT), Lo,
                                   DAG.getConstant(0, dl, NVT), ISD::SETNE);

  SDValue const LoLZ = DAG.getNode(ISD::CTTZ_ZERO_UNDEF, dl, NVT, Lo);
  SDValue const HiLZ = DAG.getNode(N->getOpcode(), dl, NVT, Hi);
...
void DAGTypeLegalizer::ExpandIntRes_FLT_ROUNDS(SDNode *N, SDValue &Lo,
  ...
  EVT const ShiftAmtTy = TLI.getShiftAmountTy(NVT, DAG.getDataLayout());
  Lo = DAG.getNode(ISD::FLT_ROUNDS_, dl, {NVT, MVT::Other}, N->getOperand(0));
  SDValue const Chain = Lo.getValue(1);
...
void DAGTypeLegalizer::ExpandIntRes_FP_TO_SINT(SDNode *N, SDValue &Lo,
  ...
  SDValue const Chain = IsStrict ? N->getOperand(0) : SDValue();
  SDValue Op = N->getOperand(IsStrict ? 1 : 0);
  if (getTypeAction(Op.getValueType()) == TargetLowering::TypePromoteFloat)
    Op = GetPromotedFloat(Op);

  if (getTypeAction(Op.getValueType()) == TargetLowering::TypeSoftPromoteHalf) {
    EVT const NFPVT = TLI.getTypeToTransformTo(*DAG.getContext(), Op.getValueType());
    Op = GetSoftPromotedHalf(Op);
    Op = DAG.getNode(ISD::FP16_TO_FP, dl, NFPVT, Op);
  }

  RTLIB::Libcall const LC = RTLIB::getFPTOSINT(Op.getValueType(), VT);
  assert(LC != RTLIB::UNKNOWN_LIBCALL && "Unexpected fp-to-sint conversion!");
  TargetLowering::MakeLibCallOptions CallOptions;
  CallOptions.setSExt(true);
  std::pair<SDValue, SDValue> const Tmp = TLI.makeLibCall(DAG, LC, VT, Op,
...
void DAGTypeLegalizer::ExpandIntRes_FP_TO_UINT(SDNode *N, SDValue &Lo,
  ...
  RTLIB::Libcall const LC = RTLIB::getFPTOUINT(Op.getValueType(), VT);
...
void DAGTypeLegalizer::ExpandIntRes_FP_TO_XINT_SAT(SDNode *N, SDValue &Lo,
                                                   SDValue &Hi) {
  SDValue const Res = TLI.expandFP_TO_INT_SAT(N, DAG);
  SplitInteger(Res, Lo, Hi);
}

void DAGTypeLegalizer::ExpandIntRes_LLROUND_LLRINT(SDNode *N, SDValue &Lo,
                                                   SDValue &Hi) {
  SDValue const Op = N->getOperand(N->isStrictFPOpcode() ? 1 : 0);
  ...
  SDLoc const dl(N);
  EVT const RetVT = N->getValueType(0);
  SDValue const Chain = N->isStrictFPOpcode() ? N->getOperand(0) : SDValue();

  TargetLowering::MakeLibCallOptions CallOptions;
  CallOptions.setSExt(true);
  std::pair<SDValue, SDValue> const Tmp = TLI.makeLibCall(DAG, LC, RetVT,
                                                    Op, CallOptions, dl,
                                                    Chain);
  SplitInteger(Tmp.first, Lo, Hi);
...
void DAGTypeLegalizer::ExpandIntRes_LOAD(LoadSDNode *N,
  ...
  if (N->isAtomic()) {
    ...
    SDLoc const dl(N);
    EVT const VT = N->getMemoryVT();
    SDVTList const VTs = DAG.getVTList(VT, MVT::i1, MVT::Other);
    SDValue const Zero = DAG.getConstant(0, dl, VT);
    SDValue const Swap = DAG.getAtomicCmpSwap(
  ...
  EVT const VT = N->getValueType(0);
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), VT);
  SDValue Ch  = N->getChain();
  SDValue Ptr = N->getBasePtr();
  ISD::LoadExtType const ExtType = N->getExtensionType();
  MachineMemOperand::Flags const MMOFlags = N->getMemOperand()->getFlags();
  AAMDNodes const AAInfo = N->getAAInfo();
  SDLoc const dl(N);
  ...
  if (N->getMemoryVT().bitsLE(NVT)) {
    ...
    if (ExtType == ISD::SEXTLOAD) {
      ...
      // lo part.
      unsigned const LoSize = Lo.getValueSizeInBits();
      Hi = DAG.getNode(ISD::SRA, dl, NVT, Lo,
    ...
    unsigned const ExcessBits =
      N->getMemoryVT().getSizeInBits() - NVT.getSizeInBits();
    EVT const NEVT = EVT::getIntegerVT(*DAG.getContext(), ExcessBits);

    // Increment the pointer to the other half.
    unsigned const IncrementSize = NVT.getSizeInBits()/8;
    ...
    EVT const MemVT = N->getMemoryVT();
    unsigned const EBytes = MemVT.getStoreSize();
    unsigned const IncrementSize = NVT.getSizeInBits()/8;
    unsigned const ExcessBits = (EBytes - IncrementSize)*8;
...
void DAGTypeLegalizer::ExpandIntRes_MUL(SDNode *N,
  ...
  if (LC == RTLIB::UNKNOWN_LIBCALL || !TLI.getLibcallName(LC)) {
    ...
    // 4.3.1).
    unsigned const Bits = NVT.getSizeInBits();
    unsigned const HalfBits = Bits >> 1;
    SDValue const Mask = DAG.getConstant(APInt::getLowBitsSet(Bits, HalfBits), dl,
                                   NVT);
    SDValue const LLL = DAG.getNode(ISD::AND, dl, NVT, LL, Mask);
    SDValue const RLL = DAG.getNode(ISD::AND, dl, NVT, RL, Mask);

    SDValue const T = DAG.getNode(ISD::MUL, dl, NVT, LLL, RLL);
    SDValue const TL = DAG.getNode(ISD::AND, dl, NVT, T, Mask);
    ...
    SDValue const Shift = DAG.getConstant(HalfBits, dl, ShiftAmtTy);
    SDValue const TH = DAG.getNode(ISD::SRL, dl, NVT, T, Shift);
    SDValue const LLH = DAG.getNode(ISD::SRL, dl, NVT, LL, Shift);
    SDValue const RLH = DAG.getNode(ISD::SRL, dl, NVT, RL, Shift);

    SDValue const U = DAG.getNode(ISD::ADD, dl, NVT,
                            DAG.getNode(ISD::MUL, dl, NVT, LLH, RLL), TH);
    SDValue const UL = DAG.getNode(ISD::AND, dl, NVT, U, Mask);
    SDValue const UH = DAG.getNode(ISD::SRL, dl, NVT, U, Shift);

    SDValue const V = DAG.getNode(ISD::ADD, dl, NVT,
                            DAG.getNode(ISD::MUL, dl, NVT, LLL, RLH), UL);
    SDValue const VH = DAG.getNode(ISD::SRL, dl, NVT, V, Shift);

    SDValue const W = DAG.getNode(ISD::ADD, dl, NVT,
...
void DAGTypeLegalizer::ExpandIntRes_READCYCLECOUNTER(SDNode *N, SDValue &Lo,
  ...
  SDLoc const DL(N);
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), N->getValueType(0));
  SDVTList const VTs = DAG.getVTList(NVT, NVT, MVT::Other);
  SDValue const R = DAG.getNode(N->getOpcode(), DL, VTs, N->getOperand(0));
  Lo = R.getValue(0);
  Hi = R.getValue(1);
...
void DAGTypeLegalizer::ExpandIntRes_ADDSUBSAT(SDNode *N, SDValue &Lo,
                                              SDValue &Hi) {
  SDValue const Result = TLI.expandAddSubSat(N, DAG);
  SplitInteger(Result, Lo, Hi);
}

void DAGTypeLegalizer::ExpandIntRes_SHLSAT(SDNode *N, SDValue &Lo,
                                           SDValue &Hi) {
  SDValue const Result = TLI.expandShlSat(N, DAG);
  SplitInteger(Result, Lo, Hi);
}
...
void DAGTypeLegalizer::ExpandIntRes_MULFIX(SDNode *N, SDValue &Lo,
  ...
  SDValue const LHS = N->getOperand(0);
  SDValue const RHS = N->getOperand(1);
  uint64_t const Scale = N->getConstantOperandVal(2);
  bool const Saturating = (N->getOpcode() == ISD::SMULFIXSAT ||
                     N->getOpcode() == ISD::UMULFIXSAT);
  bool const Signed = (N->getOpcode() == ISD::SMULFIX ||
  ...
  if (!Scale) {
    ...
    if (!Saturating) {
      ...
      EVT const BoolVT = getSetCCResultType(VT);
      unsigned const MulOp = Signed ? ISD::SMULO : ISD::UMULO;
  ...
  unsigned const NVTSize = NVT.getScalarSizeInBits();
  assert((VTSize == NVTSize * 2) && "Expected the new value type to be half "
                                    "the size of the current value type");
  EVT const ShiftTy = TLI.getShiftAmountTy(NVT, DAG.getDataLayout());
  ...
  // shifting.
  uint64_t const Part0 = Scale / NVTSize; // Part holding lowest bit needed.
  if (Scale % NVTSize) {
    SDValue const ShiftAmount = DAG.getConstant(Scale % NVTSize, dl, ShiftTy);
  ...
  SDValue const ResultHL = Result[2];
  SDValue const ResultHH = Result[3];

  SDValue SatMax, SatMin;
  SDValue const NVTZero = DAG.getConstant(0, dl, NVT);
  SDValue const NVTNeg1 = DAG.getConstant(-1, dl, NVT);
  EVT const BoolNVT = getSetCCResultType(NVT);

  if (!Signed) {
    if (Scale < NVTSize) {
      // Overflow happened if ((HH | (HL >> Scale)) != 0).
      SDValue const HLAdjusted = DAG.getNode(ISD::SRL, dl, NVT, ResultHL,
                                       DAG.getConstant(Scale, dl, ShiftTy));
      SDValue const Tmp = DAG.getNode(ISD::OR, dl, NVT, HLAdjusted, ResultHH);
  ...
  if (Scale < NVTSize) {
    ...
    unsigned const OverflowBits = VTSize - Scale + 1;
    assert(OverflowBits <= VTSize && OverflowBits > NVTSize &&
           "Extent of overflow bits must start within HL");
    SDValue const HLHiMask = DAG.getConstant(
        APInt::getHighBitsSet(NVTSize, OverflowBits - NVTSize), dl, NVT);
    SDValue const HLLoMask = DAG.getConstant(
        APInt::getLowBitsSet(NVTSize, VTSize - OverflowBits), dl, NVT);
    // We overflow max if HH > 0 or (HH == 0 && HL > HLLoMask).
    SDValue const HHGT0 = DAG.getSetCC(dl, BoolNVT, ResultHH, NVTZero, ISD::SETGT);
    SDValue const HHEQ0 = DAG.getSetCC(dl, BoolNVT, ResultHH, NVTZero, ISD::SETEQ);
    SDValue const HLUGT = DAG.getSetCC(dl, BoolNVT, ResultHL, HLLoMask, ISD::SETUGT);
    ...
    SDValue const HHLT = DAG.getSetCC(dl, BoolNVT, ResultHH, NVTNeg1, ISD::SETLT);
    SDValue const HHEQ = DAG.getSetCC(dl, BoolNVT, ResultHH, NVTNeg1, ISD::SETEQ);
    SDValue const HLULT = DAG.getSetCC(dl, BoolNVT, ResultHL, HLHiMask, ISD::SETULT);
    ...
    SDValue const HHGT0 = DAG.getSetCC(dl, BoolNVT, ResultHH, NVTZero, ISD::SETGT);
    SDValue const HHEQ0 = DAG.getSetCC(dl, BoolNVT, ResultHH, NVTZero, ISD::SETEQ);
    SDValue const HLNeg = DAG.getSetCC(dl, BoolNVT, ResultHL, NVTZero, ISD::SETLT);
    SatMax = DAG.getNode(ISD::OR, dl, BoolNVT, HHGT0,
                         DAG.getNode(ISD::AND, dl, BoolNVT, HHEQ0, HLNeg));
    // We overflow min if HH < -1 or (HH == -1 && HL sign bit is 0).
    SDValue const HHLT = DAG.getSetCC(dl, BoolNVT, ResultHH, NVTNeg1, ISD::SETLT);
    SDValue const HHEQ = DAG.getSetCC(dl, BoolNVT, ResultHH, NVTNeg1, ISD::SETEQ);
    SDValue const HLPos = DAG.getSetCC(dl, BoolNVT, ResultHL, NVTZero, ISD::SETGE);
    SatMin = DAG.getNode(ISD::OR, dl, BoolNVT, HHLT,
    ...
    unsigned const OverflowBits = VTSize - Scale + 1;
    SDValue const HHHiMask = DAG.getConstant(
        APInt::getHighBitsSet(NVTSize, OverflowBits), dl, NVT);
    SDValue const HHLoMask = DAG.getConstant(
  ...
  APInt const MaxHi = APInt::getSignedMaxValue(NVTSize);
  APInt const MaxLo = APInt::getAllOnesValue(NVTSize);
  ...
  APInt const MinHi = APInt::getSignedMinValue(NVTSize);
...
void DAGTypeLegalizer::ExpandIntRes_SADDSUBO(SDNode *Node,
  ...
  unsigned const CarryOp = IsAdd ? ISD::SADDO_CARRY : ISD::SSUBO_CARRY;
  ...
  if (HasCarryOp) {
    ...
    GetExpandedInteger(LHS, LHSL, LHSH);
    GetExpandedInteger(RHS, RHSL, RHSH);
    SDVTList const VTList = DAG.getVTList(LHSL.getValueType(), Node->getValueType(1));
    ...
    // Expand the result by simply replacing it with the equivalent
    // non-overflow-checking operation.
    SDValue const Sum = DAG.getNode(Node->getOpcode() == ISD::SADDO ?
    ...
    SDValue const SumSignNE = DAG.getNode(ISD::XOR, dl, VT, LHS, Sum);
    Ovf = DAG.getNode(ISD::AND, dl, VT, SignsMatch, SumSignNE);
    EVT const OType = Node->getValueType(1);
...
void DAGTypeLegalizer::ExpandIntRes_SDIV(SDNode *N,
  ...
  if (TLI.getOperationAction(ISD::SDIVREM, VT) == TargetLowering::Custom) {
    SDValue const Res = DAG.getNode(ISD::SDIVREM, dl, DAG.getVTList(VT, VT), Ops);
...
void DAGTypeLegalizer::ExpandIntRes_Shift(SDNode *N,
  ...
  EVT const NVT = TLI.getTypeToTransformTo(*DAG.getContext(), VT);
  TargetLowering::LegalizeAction const Action = TLI.getOperationAction(PartsOpc, NVT);
  ...
  if (LegalOrCustom && TLI.shouldExpandShift(DAG, N)) {
    ...
    SDValue const Ops[] = { LHSL, LHSH, ShiftOp };
...
void DAGTypeLegalizer::ExpandIntRes_SIGN_EXTEND(SDNode *N,
  ...
  if (Op.getValueType().bitsLE(NVT)) {
    ...
    unsigned const LoSize = NVT.getSizeInBits();
    Hi = DAG.getNode(
        ISD::SRA, dl, NVT, Lo,
    ...
    // Split the promoted operand.  This will simplify when it is expanded.
    SplitInteger(Res, Lo, Hi);
    unsigned const ExcessBits = Op.getValueSizeInBits() - NVT.getSizeInBits();
...
ExpandIntRes_SIGN_EXTEND_INREG(SDNode *N, SDValue &Lo, SDValue &Hi) {
  ...
  if (EVT.bitsLE(Lo.getValueType())) {
    ...
    unsigned const ExcessBits = EVT.getSizeInBits() - Lo.getValueSizeInBits();
    Hi = DAG.getNode(ISD::SIGN_EXTEND_INREG, dl, Hi.getValueType(), Hi,
                     DAG.getValueType(EVT::getIntegerVT(*DAG.getContext(),
                                                        ExcessBits)));
  }
...
void DAGTypeLegalizer::ExpandIntRes_XMULO(SDNode *N,
  ...
  if (N->getOpcode() == ISD::UMULO) {
    ...
    EVT const HalfVT = LHSLow.getValueType();
    EVT const BitVT = N->getValueType(1);
    SDVTList const VTHalfWithO = DAG.getVTList(HalfVT, BitVT);

    SDValue const HalfZero = DAG.getConstant(0, dl, HalfVT);
    ...
    SDValue const One = DAG.getNode(ISD::UMULO, dl, VTHalfWithO, LHSHigh, RHSLow);
    Overflow = DAG.getNode(ISD::OR, dl, BitVT, Overflow, One.getValue(1));

    SDValue const Two = DAG.getNode(ISD::UMULO, dl, VTHalfWithO, RHSHigh, LHSLow);
    Overflow = DAG.getNode(ISD::OR, dl, BitVT, Overflow, Two.getValue(1));

    SDValue const HighSum = DAG.getNode(ISD::ADD, dl, HalfVT, One, Two);
    ...
    SDValue const Three = DAG.getNode(ISD::MUL, dl, VT,
  ...
  EVT const PtrVT = TLI.getPointerTy(DAG.getDataLayout());
  ...
  if (LC == RTLIB::UNKNOWN_LIBCALL || !TLI.getLibcallName(LC)) {
    ...
    SDValue const LHS = DAG.getNode(ISD::SIGN_EXTEND, dl, WideVT, N->getOperand(0));
    SDValue const RHS = DAG.getNode(ISD::SIGN_EXTEND, dl, WideVT, N->getOperand(1));
  ...
  SDValue const Temp = DAG.CreateStackTemporary(PtrVT);
  ...
  for (const SDValue &Op : N->op_values()) {
    EVT const ArgVT = Op.getValueType();
  ...
  SDValue const Func = DAG.getExternalSymbol(TLI.getLibcallName(LC), PtrVT);
  ...
  std::pair<SDValue, SDValue> const CallInfo = TLI.LowerCallTo(CLI);

  SplitInteger(CallInfo.first, Lo, Hi);
  SDValue const Temp2 =
      DAG.getLoad(PtrVT, dl, CallInfo.second, Temp, MachinePointerInfo());
  SDValue const Ofl = DAG.getSetCC(dl, N->getValueType(1), Temp2,
...
void DAGTypeLegalizer::ExpandIntRes_UDIV(SDNode *N,
  ...
  if (TLI.getOperationAction(ISD::UDIVREM, VT) == TargetLowering::Custom) {
    SDValue const Res = DAG.getNode(ISD::UDIVREM, dl, DAG.getVTList(VT, VT), Ops);
    SplitInteger(Res.getValue(0), Lo, Hi);
    return;
  }
...
void DAGTypeLegalizer::ExpandIntRes_ATOMIC_LOAD(SDNode *N,
                                                SDValue &Lo, SDValue &Hi) {
  SDLoc const dl(N);
  EVT const VT = cast<AtomicSDNode>(N)->getMemoryVT();
  SDVTList const VTs = DAG.getVTList(VT, MVT::i1, MVT::Other);
  SDValue const Zero = DAG.getConstant(0, dl, VT);
  SDValue const Swap = DAG.getAtomicCmpSwap(
      ISD::ATOMIC_CMP_SWAP_WITH_SUCCESS, dl,
...
void DAGTypeLegalizer::ExpandIntRes_VECREDUCE(SDNode *N,
  ...
  SDValue const Res = TLI.expandVecReduce(N, DAG);
  SplitInteger(Res, Lo, Hi);
}
...
void DAGTypeLegalizer::ExpandIntRes_VSCALE(SDNode *N, SDValue &Lo,
  ...
  APInt const One(HalfVT.getSizeInBits(), 1);
  SDValue VScaleBase = DAG.getVScale(dl, HalfVT, One);
  VScaleBase = DAG.getNode(ISD::ZERO_EXTEND, dl, VT, VScaleBase);
  SDValue const Res = DAG.getNode(ISD::MUL, dl, VT, VScaleBase, N->getOperand(0));
  SplitInteger(Res, Lo, Hi);
}
...
void DAGTypeLegalizer::IntegerExpandSetCCOperands(SDValue &NewLHS,
  ...
  bool const EqAllowed = (CCCode == ISD::SETLE || CCCode == ISD::SETGE ||
  ...
  EVT const HiVT = LHSHi.getValueType();
  EVT const ExpandVT = TLI.getTypeToExpandTo(*DAG.getContext(), HiVT);
  bool const HasSETCCCARRY = TLI.isOperationLegalOrCustom(ISD::SETCCCARRY, ExpandVT);
  ...
  if (HasSETCCCARRY) {
    ...
    EVT const LoVT = LHSLo.getValueType();
    SDVTList const VTList = DAG.getVTList(LoVT, getSetCCResultType(LoVT));
    SDValue const LowCmp = DAG.getNode(ISD::USUBO, dl, VTList, LHSLo, RHSLo);
    SDValue const Res = DAG.getNode(ISD::SETCCCARRY, dl, getSetCCResultType(HiVT),
...
SDValue DAGTypeLegalizer::ExpandIntOp_SETCCCARRY(SDNode *N) {
  ...
  SDVTList const VTList = DAG.getVTList(LHSLo.getValueType(), Carry.getValueType());
  SDValue const LowCmp = DAG.getNode(ISD::SUBCARRY, dl, VTList, LHSLo, RHSLo, Carry);
...
SDValue DAGTypeLegalizer::ExpandIntOp_SINT_TO_FP(SDNode *N) {
  bool const IsStrict = N->isStrictFPOpcode();
  SDValue const Chain = IsStrict ? N->getOperand(0) : SDValue();
  SDValue const Op = N->getOperand(IsStrict ? 1 : 0);
  EVT const DstVT = N->getValueType(0);
  RTLIB::Libcall const LC = RTLIB::getSINTTOFP(Op.getValueType(), DstVT);
  assert(LC != RTLIB::UNKNOWN_LIBCALL &&
...
SDValue DAGTypeLegalizer::ExpandIntOp_STORE(StoreSDNode *N, unsigned OpNo) {
  if (N->isAtomic()) {
    ...
    SDLoc const dl(N);
    SDValue const Swap = DAG.getAtomic(ISD::ATOMIC_SWAP, dl,
                                 N->getMemoryVT(),
  ...
  EVT const ExtVT = N->getMemoryVT();
  unsigned const EBytes = ExtVT.getStoreSize();
  unsigned const IncrementSize = NVT.getSizeInBits()/8;
  unsigned const ExcessBits = (EBytes - IncrementSize)*8;
  EVT const HiVT = EVT::getIntegerVT(*DAG.getContext(),
...
SDValue DAGTypeLegalizer::ExpandIntOp_UINT_TO_FP(SDNode *N) {
  ...
  SDValue const Op = N->getOperand(IsStrict ? 1 : 0);
  EVT const DstVT = N->getValueType(0);
  RTLIB::Libcall const LC = RTLIB::getUINTTOFP(Op.getValueType(), DstVT);
  assert(LC != RTLIB::UNKNOWN_LIBCALL &&
...
SDValue DAGTypeLegalizer::PromoteIntRes_VECTOR_SPLICE(SDNode *N) {
  ...
  SDValue const V0 = GetPromotedInteger(N->getOperand(0));
  SDValue const V1 = GetPromotedInteger(N->getOperand(1));
  EVT const OutVT = V0.getValueType();
...
SDValue DAGTypeLegalizer::PromoteIntRes_EXTRACT_SUBVECTOR(SDNode *N) {
  ...
  EVT const NOutVTElem = NOutVT.getVectorElementType();

  SDLoc const dl(N);
  SDValue const BaseIdx = N->getOperand(1);
  ...
  if (OutVT.isScalableVector()) {
    SDValue const InOp0 = N->getOperand(0);
    EVT const InVT = InOp0.getValueType();
    ...
    if (getTypeAction(InVT) == TargetLowering::TypePromoteInteger) {
      ...
      SDValue const Ops[] = { GetPromotedInteger(InOp0), BaseIdx };

      EVT const PromEltVT = Ops[0].getValueType().getVectorElementType();
      assert(PromEltVT.bitsLE(NOutVTElem) &&
             "Promoted operand has an element type greater than result");

      EVT const ExtVT = NOutVT.changeVectorElementType(PromEltVT);
      SDValue const Ext = DAG.getNode(ISD::EXTRACT_SUBVECTOR, SDLoc(N), ExtVT, Ops);
  ...
  unsigned const OutNumElems = OutVT.getVectorNumElements();
  SmallVector<SDValue, 8> Ops;
  Ops.reserve(OutNumElems);
  for (unsigned i = 0; i != OutNumElems; ++i) {

    // Extract the element from the original vector.
    SDValue const Index = DAG.getNode(ISD::ADD, dl, BaseIdx.getValueType(),
      BaseIdx, DAG.getConstant(i, dl, BaseIdx.getValueType()));
    SDValue const Ext = DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl,
      InVT.getVectorElementType(), N->getOperand(0), Index);

    SDValue const Op = DAG.getAnyExtOrTrunc(Ext, dl, NOutVTElem);
...
SDValue DAGTypeLegalizer::PromoteIntRes_VECTOR_SHUFFLE(SDNode *N) {
  ...
  ArrayRef<int> const NewMask = SV->getMask().slice(0, VT.getVectorNumElements());
...
SDValue DAGTypeLegalizer::PromoteIntRes_SCALAR_TO_VECTOR(SDNode *N) {
  ...
  SDValue const Op = DAG.getNode(ISD::ANY_EXTEND, dl, NOutVTElem, N->getOperand(0));
...
SDValue DAGTypeLegalizer::PromoteIntRes_SPLAT_VECTOR(SDNode *N) {
  ...
  SDValue const SplatVal = N->getOperand(0);
  ...
  EVT const NOutElemVT = NOutVT.getVectorElementType();

  SDValue const Op = DAG.getNode(ISD::ANY_EXTEND, dl, NOutElemVT, SplatVal);
...
SDValue DAGTypeLegalizer::PromoteIntRes_STEP_VECTOR(SDNode *N) {
  ...
  EVT const NOutVT = TLI.getTypeToTransformTo(*DAG.getContext(), OutVT);
  assert(NOutVT.isVector() && "Type must be promoted to a vector type");
  APInt const StepVal = cast<ConstantSDNode>(N->getOperand(0))->getAPIntValue();
...
SDValue DAGTypeLegalizer::PromoteIntRes_CONCAT_VECTORS(SDNode *N) {
  ...
  EVT const OutElemTy = NOutVT.getVectorElementType();

  unsigned const NumElem = N->getOperand(0).getValueType().getVectorNumElements();
  unsigned const NumOutElem = NOutVT.getVectorNumElements();
  ...
  for (unsigned i = 0; i < NumOperands; ++i) {
    ...
    EVT const SclrTy = Op.getValueType().getVectorElementType();
    assert(NumElem == Op.getValueType().getVectorNumElements() &&
           "Unexpected number of elements");

    for (unsigned j = 0; j < NumElem; ++j) {
      SDValue const Ext = DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl, SclrTy, Op,
...
SDValue DAGTypeLegalizer::PromoteIntRes_INSERT_VECTOR_ELT(SDNode *N) {
  ...
  SDValue const ConvElem = DAG.getNode(ISD::ANY_EXTEND, dl,
...
SDValue DAGTypeLegalizer::PromoteIntOp_EXTRACT_VECTOR_ELT(SDNode *N) {
  SDLoc const dl(N);
  SDValue const V0 = GetPromotedInteger(N->getOperand(0));
  SDValue const V1 = DAG.getZExtOrTrunc(N->getOperand(1), dl,
                                  TLI.getVectorIdxTy(DAG.getDataLayout()));
  SDValue const Ext = DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl,
...
SDValue DAGTypeLegalizer::PromoteIntOp_EXTRACT_SUBVECTOR(SDNode *N) {
  SDLoc const dl(N);
  SDValue const V0 = GetPromotedInteger(N->getOperand(0));
  MVT const InVT = V0.getValueType().getSimpleVT();
  MVT const OutVT = MVT::getVectorVT(InVT.getVectorElementType(),
                               N->getValueType(0).getVectorNumElements());
  SDValue const Ext = DAG.getNode(ISD::EXTRACT_SUBVECTOR, dl, OutVT, V0, N->getOperand(1));
...
SDValue DAGTypeLegalizer::PromoteIntOp_CONCAT_VECTORS(SDNode *N) {
  ...
  if (ResVT.isScalableVector()) {
    ...
    for (unsigned OpIdx = 0; OpIdx < NumElems; ++OpIdx) {
      SDValue const Op = N->getOperand(OpIdx);
      unsigned const OpNumElts = Op.getValueType().getVectorMinNumElements();
  ...
  EVT const RetSclrTy = N->getValueType(0).getVectorElementType();
  ...
  for (unsigned VecIdx = 0; VecIdx != NumElems; ++VecIdx) {
    SDValue const Incoming = GetPromotedInteger(N->getOperand(VecIdx));
    EVT const SclrTy = Incoming->getValueType(0).getVectorElementType();
    unsigned const NumElem = Incoming->getValueType(0).getVectorNumElements();

    for (unsigned i=0; i<NumElem; ++i) {
      // Extract element from incoming vector
      SDValue const Ex = DAG.getNode(ISD::EXTRACT_VECTOR_ELT, dl, SclrTy, Incoming,
                               DAG.getVectorIdxConstant(i, dl));
      SDValue const Tr = DAG.getNode(ISD::TRUNCATE, dl, RetSclrTy, Ex);
      NewOps.push_back(Tr);
    }
  }
