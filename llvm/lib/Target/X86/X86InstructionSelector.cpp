  ...
  if (DstReg.isPhysical()) {
    ...
    if (DstSize > SrcSize && SrcRegBank.getID() == X86::GPRRegBankID &&
      ...
      if (SrcRC != DstRC) {
        ...
        Register const ExtSrc = MRI.createVirtualRegister(DstRC);
...
unsigned X86InstructionSelector::getLoadStoreOp(const LLT &Ty,
                                                ...
                                                unsigned Opc,
                                                Align Alignment) const {
  bool const Isload = (Opc == TargetOpcode::G_LOAD);
...
bool X86InstructionSelector::selectLoadStoreOp(MachineInstr &I,
  ...
  unsigned const NewOpc = getLoadStoreOp(Ty, RB, Opc, MemOp.getAlign());
  if (NewOpc == Opc)
    return false;
...
bool X86InstructionSelector::selectFrameIndexOrGep(MachineInstr &I,
  ...
  unsigned const NewOpc = getLeaOP(Ty, STI);
  I.setDesc(TII.get(NewOpc));
  MachineInstrBuilder const MIB(MF, I);
...
bool X86InstructionSelector::selectFCmp(MachineInstr &I,
  ...
  CmpInst::Predicate const Predicate =
      (CmpInst::Predicate)I.getOperand(1).getPredicate();
  ...
  unsigned OpCmp;
  LLT const Ty = MRI.getType(LhsReg);
  switch (Ty.getSizeInBits()) {
  default:
    return false;
  case 32:
  ...
  Register const ResultReg = I.getOperand(0).getReg();
  ...
  if (SETFOpc) {
    ...
    Register const FlagReg1 = MRI.createVirtualRegister(&X86::GR8RegClass);
    Register const FlagReg2 = MRI.createVirtualRegister(&X86::GR8RegClass);
...
bool X86InstructionSelector::selectUnmergeValues(
  ...
  // Split to extracts.
  unsigned const NumDefs = I.getNumOperands() - 1;
  Register const SrcReg = I.getOperand(NumDefs).getReg();
  unsigned const DefSize = MRI.getType(I.getOperand(0).getReg()).getSizeInBits();
...
bool X86InstructionSelector::selectMergeValues(
  ...
  // Split to inserts.
  Register const DstReg = I.getOperand(0).getReg();
  Register const SrcReg0 = I.getOperand(1).getReg();
  ...
  for (unsigned Idx = 2; Idx < I.getNumOperands(); ++Idx) {
    Register const Tmp = MRI.createGenericVirtualRegister(DstTy);
...
bool X86InstructionSelector::materializeFP(MachineInstr &I,
  ...
  const LLT DstTy = MRI.getType(DstReg);
  const RegisterBank &RegBank = *RBI.getRegBank(DstReg, MRI, TRI);
  Align const Alignment = Align(DstTy.getSizeInBytes());
  ...
  unsigned const CPI = MF.getConstantPool()->getConstantPoolIndex(CFP, Alignment);
  MachineInstr *LoadInst = nullptr;
  unsigned char const OpFlag = STI.classifyLocalReference(nullptr);
...
bool X86InstructionSelector::selectDivRem(MachineInstr &I,
  ...
  if (OpEntry.OpSignExtend) {
    if (OpEntry.IsOpSigned)
      ...
      Register const Zero32 = MRI.createVirtualRegister(&X86::GR32RegClass);
  ...
  if ((I.getOpcode() == Instruction::SRem ||
    ...
    Register const SourceSuperReg = MRI.createVirtualRegister(&X86::GR16RegClass);
    Register const ResultSuperReg = MRI.createVirtualRegister(&X86::GR16RegClass);
