...
static int insertNamedMCOperand(MCInst &MI, const MCOperand &Op,
                                uint16_t NameIdx) {
  int const OpIdx = AMDGPU::getNamedOperandIdx(MI.getOpcode(), NameIdx);
  if (OpIdx != -1) {
    auto I = MI.begin();
    std::advance(I, OpIdx);
    MI.insert(I, Op);
...
static DecodeStatus decodeSoppBrTarget(MCInst &Inst, unsigned Imm,
  ...
  int64_t const Offset = (SignedOffset.sext(64) + 4 + Addr).getSExtValue();
...
static DecodeStatus decodeOperand_AVLdSt_Any(MCInst &Inst,
  ...
  if (!DAsm->isGFX90A()) {
    ...
    uint64_t const TSFlags = DAsm->getMCII()->get(Opc).TSFlags;
    ...
    int const DataIdx = AMDGPU::getNamedOperandIdx(Opc, DataNameIdx);
    if ((int)Inst.getNumOperands() == DataIdx) {
      int const DstIdx = AMDGPU::getNamedOperandIdx(Opc, AMDGPU::OpName::vdst);
      if (IsAGPROperand(Inst, DstIdx, MRI))
        Imm |= 512;
    }

    if (TSFlags & SIInstrFlags::DS) {
      int const Data2Idx = AMDGPU::getNamedOperandIdx(Opc, AMDGPU::OpName::data1);
...
static bool isValidDPP8(const MCInst &MI) {
  ...
  int const FiIdx = AMDGPU::getNamedOperandIdx(MI.getOpcode(), AMDGPU::OpName::fi);
  assert(FiIdx != -1);
  if ((unsigned)FiIdx >= MI.getNumOperands())
    return false;
  unsigned const Fi = MI.getOperand(FiIdx).getImm();
...
DecodeStatus AMDGPUDisassembler::getInstruction(MCInst &MI, uint64_t &Size,
  ...
  unsigned const MaxInstBytesNum = std::min((size_t)TargetMaxInstBytes, Bytes_.size());
  ...
  if (Res && (MCII->get(MI.getOpcode()).TSFlags &
    ...
    int const CPolPos = AMDGPU::getNamedOperandIdx(MI.getOpcode(),
                                             AMDGPU::OpName::cpol);
    if (CPolPos != -1) {
      unsigned const CPol =
  ...
  if (Res && (MCII->get(MI.getOpcode()).TSFlags & SIInstrFlags::MIMG)) {
    ...
    unsigned const NSAArgs = RsrcIdx - VAddr0Idx - 1;
    if (VAddr0Idx >= 0 && NSAArgs > 0) {
      unsigned const NSAWords = (NSAArgs + 3) / 4;
  ...
  int const VDstIn_Idx = AMDGPU::getNamedOperandIdx(MI.getOpcode(),
                                              AMDGPU::OpName::vdst_in);
  if (VDstIn_Idx != -1) {
    int const Tied = MCII->get(MI.getOpcode()).getOperandConstraint(VDstIn_Idx,
...
DecodeStatus AMDGPUDisassembler::convertSDWAInst(MCInst &MI) const {
  if (STI.getFeatureBits()[AMDGPU::FeatureGFX9] ||
    ...
    int const SDst = AMDGPU::getNamedOperandIdx(MI.getOpcode(), AMDGPU::OpName::sdst);
    if (SDst != -1) {
...
DecodeStatus AMDGPUDisassembler::convertDPP8Inst(MCInst &MI) const {
  ...
  unsigned const DescNumOps = MCII->get(Opc).getNumOperands();
...
DecodeStatus AMDGPUDisassembler::convertMIMGInst(MCInst &MI) const {
  ...
  int const VDstIdx = AMDGPU::getNamedOperandIdx(MI.getOpcode(),
                                           AMDGPU::OpName::vdst);

  int const VDataIdx = AMDGPU::getNamedOperandIdx(MI.getOpcode(),
                                            AMDGPU::OpName::vdata);
  int const VAddr0Idx =
      AMDGPU::getNamedOperandIdx(MI.getOpcode(), AMDGPU::OpName::vaddr0);
  int const DMaskIdx = AMDGPU::getNamedOperandIdx(MI.getOpcode(),
                                            AMDGPU::OpName::dmask);

  int const TFEIdx   = AMDGPU::getNamedOperandIdx(MI.getOpcode(),
                                            AMDGPU::OpName::tfe);
  int const D16Idx   = AMDGPU::getNamedOperandIdx(MI.getOpcode(),
                                            AMDGPU::OpName::d16);
  ...
  bool const IsAtomic = (VDstIdx != -1);
  bool const IsGather4 = MCII->get(MI.getOpcode()).TSFlags & SIInstrFlags::Gather4;

  bool IsNSA = false;
  unsigned AddrSize = Info->VAddrDwords;

  if (STI.getFeatureBits()[AMDGPU::FeatureGFX10]) {
    unsigned const DimIdx =
  ...
  unsigned const DMask = MI.getOperand(DMaskIdx).getImm() & 0xf;
  unsigned DstSize = IsGather4 ? 4 : std::max(countPopulation(DMask), 1u);

  bool const D16 = D16Idx >= 0 && MI.getOperand(D16Idx).getImm();
  ...
  if (DstSize != Info->VDataDwords) {
    ...
    unsigned const VdataSub0 = MRI.getSubReg(Vdata0, AMDGPU::sub0);
  ...
  if (STI.getFeatureBits()[AMDGPU::FeatureGFX10] && !IsNSA &&
    ...
    unsigned const VAddrSub0 = MRI.getSubReg(VAddr0, AMDGPU::sub0);
...
int AMDGPUDisassembler::getTTmpIdx(unsigned Val) const {
  ...
  unsigned const TTmpMin = isGFX9Plus() ? TTMP_GFX9PLUS_MIN : TTMP_VI_MIN;
  unsigned const TTmpMax = isGFX9Plus() ? TTMP_GFX9PLUS_MAX : TTMP_VI_MAX;
...
MCOperand AMDGPUDisassembler::decodeSrcOp(const OpWidthTy Width, unsigned Val) const {
  ...
  int const TTmpIdx = getTTmpIdx(Val);
  if (TTmpIdx >= 0) {
...
MCOperand AMDGPUDisassembler::decodeSDWAVopcDst(unsigned Val) const {
  ...
  bool const IsWave64 = STI.getFeatureBits()[AMDGPU::FeatureWavefrontSize64];
...
MCDisassembler::DecodeStatus AMDGPUDisassembler::decodeCOMPUTE_PGM_RSRC1(
  ...
  uint32_t const GranulatedWavefrontSGPRCount =
  ...
  uint32_t const NextFreeSGPR = (GranulatedWavefrontSGPRCount + 1) *
...
MCDisassembler::DecodeStatus AMDGPUDisassembler::decodeKernelDescriptor(
  ...
  while (C && C.tell() < Bytes.size()) {
    MCDisassembler::DecodeStatus const Status =
...
AMDGPUDisassembler::onSymbolStart(SymbolInfoTy &Symbol, uint64_t &Size,
  ...
  StringRef const Name = Symbol.Name;
